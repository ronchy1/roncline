...
export class McpHub {
  ...
  "mcpServers": {
	...
	public async updateServerHeadersRPC(serverName: string, headers: Record<string, string>): Promise<McpServer[]> {
		try {
			// Validate headers
			...
			console.error("Failed to update server headers:", error)
			if (error instanceof Error) {
				console.error("Error details:", error.message, error.stack)
			}
			HostProvider.window.showMessage({
				type: ShowMessageType.ERROR,
				message: `Failed to update server headers: ${error instanceof Error ? error.message : String(error)}`,
			})
			throw error
		}
	}
